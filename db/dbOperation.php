...
class DBOperation
{
    ...
    public function selectPostSearch($search)
    {
        $sql = 'SELECT * FROM `posts` WHERE `title` LIKE ? OR `datetime` LIKE ? OR `category` LIKE ? OR `author` LIKE ? OR `post` LIKE ? ORDER BY `id` ';
        $stmt = $this->conn->prepare($sql);
    ...
    public function insertComment($dateTime, $username, $email, $comment, $approvedBy, $status, $postID)
    {
        ...
        $stmt->bindParam(7, $postID);$stmt->bindParam(1, $dateTime);

        return $stmt->execute();
    }

    public function selectComments($status, $postID)
    {
